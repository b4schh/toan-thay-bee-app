  ...
  if (loading) {
    return (
      <View style={styles.mainContainer}>
        <AppText style={styles.headerText}>Đang tải...</AppText>
      </View>
    );
  }
              ...
              return (
                ...
                >
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    ...
                    {isCorrect === true && <Text style={{ color: 'green', fontSize: 18 }}>✓</Text>}
                  </View>
                  ...
                  {question.imageUrl && (
                    <>
                      <Text>{question.imageUrl}</Text>
                      <Image
                        source={{ uri: question.solutionUrl }}
                  ...
                  <View style={{ flexDirection: 'column', gap: 8 }}>
                    {question.statements.map((statement, index) => {
                      ...
                      return (
                        <View key={statement.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                          <Text style={{ fontWeight: 'bold', color }}>{prefixStatementTN[index]}</Text>
                          <Text style={{ flex: 1, color }}>{statement.content}</Text>
                          {icon && (
                            <Text style={{ marginLeft: 8, fontWeight: 'bold', color, fontSize: 16 }}>{icon}</Text>
                          )}
                        </View>
                      );
                    })}
                  ...
                  {shownSolutions[question.id] && (
                    ...
                    >
                      <Text style={{ fontWeight: 'bold', fontSize: 14 }}>Lời giải:</Text>

                      {question.solution ? (
                        <Text style={{ whiteSpace: 'pre-line' }}>{question.solution}</Text>
                      ) : null}
          ...
          <View style={{ flexDirection: 'column', gap: 16 }}>
            {questionDS.map((question, idx) => {
              return (
                ...
                >
                  {/* Câu hỏi */}
                  <Text style={{ fontWeight: 'bold', fontSize: 16 }}>Câu {idx + 1}:</Text>
                  <Text>{question.content}</Text>

                  {question.imageUrl && (
                    <Image
                      source={{ uri: question.imageUrl }}
                      style={{ width: '100%', height: 180, resizeMode: 'contain', borderRadius: 8 }}
                    />
                  )}
                      ...
                      return (
                        ...
                        >
                          ...
                          <View style={{ flexDirection: 'row', gap: 6, alignItems: 'flex-start' }}>
                            <Text style={{ fontWeight: 'bold' }}>{prefixStatementDS[index]}</Text>
                            <View style={{ flex: 1 }}>
                          ...
                          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }}>
                            <Text>Trả lời: <Text style={{ fontWeight: 'bold' }}>
                              {userAnswer === true ? 'Đúng' : userAnswer === false ? 'Sai' : 'Không trả lời'}
                            </Text></Text>
                            <Text>Đáp án: <Text style={{ fontWeight: 'bold' }}>
                              {statement.isCorrect === true ? 'Đúng' : 'Sai'}
                            </Text></Text>
                            <Text style={{ fontWeight: 'bold', color: isCorrect ? 'green' : 'red' }}>
                              {isCorrect ? '✓ Đúng' : '✗ Sai'}
                            </Text>
                          </View>
                        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={{ flexDirection: 'column', gap: 16 }}>
            {questionTLN.map((question, idx) => {
              ...
              const userAnswer = userAnswerRaw.trim() !== '' ? JSON.parse(userAnswerRaw) : '';
              const isCorrect = answersTNTLN[question.id]?.result;
              ...
              return (
                ...
                >
                  ...
                  >
                    <Text style={{ fontSize: 13, color: '#666', fontWeight: '500', marginBottom: 4 }}>
                      Câu trả lời của bạn:
                    </Text>
                  ...
                  {correctAnswer !== '' && (
                    ...
                    >
                      <Text style={{ fontSize: 13, color: 'green', fontWeight: '500', marginBottom: 4 }}>
                        Đáp án đúng:
                      </Text>
